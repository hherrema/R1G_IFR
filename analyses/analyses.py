...
def pfr_sess(data, ll):
    ...
    pfr /= (len(data.list.unique()) - subtract)         # divide by lists in session
    
    return pfr

# only trials initiated with correct recall
def pfr_sess_only_cr(data, ll):
    pfr = np.zeros(ll)
    rec_evs = data[data['type'] == 'REC_WORD']
    ...
    for i in data.list.unique():
        sp = rec_evs[rec_evs['list'] == i].serial_position.to_numpy()        # current list serial positions
        if len(sp) > 1 and sp[0] > 0 and sp[0] <= ll:     # only lists with correct first recall
            pfr[int(sp[0] - 1)] += 1
        else:   # no recalls or first recall intrusion, subtract lists
            subtract += 1
    
    pfr /= (len(data.list.unique()) - subtract)         # divide by lists in session
    return pfr

def pfr_all(df, condition_map, fxn):
    pfr_data_all = []
    for (sub, sess, c, ll, pr), data in tqdm(df.groupby(['worker_id', 'session', 'condition', 'l_length', 'pres_rate'])):
        pfr = fxn(data, int(ll))
...
def r1_sp_dec_btwn_subj_avg(r1_decile_data):
    r1_sp_dec_data_bsa = []
...
# all trials (including R1 intrusions)
### THINK ABOUT REMOVING R1 INTRUSIONS HERE INSTEAD OF IN PLOTS
def rt_init_all_trials_sess(data, sub, strat, sess, c, ll, pr):
...
def rt_init_all_trials(df, condition_map):
    rti_at_data = []
    for (sub, strat, sess, c, ll, pr), data in tqdm(df.groupby(['worker_id', 'strategy', 'session', 'condition', 'l_length', 'pres_rate'])):
        rts = rt_init_all_trials_sess(data, sub, strat, sess, condition_map.get(c), ll, pr)
...
def rt_init(df, condition_map, toggle=True):
    rti_data = []
    for (sub, strat, sess, c, ll, pr), data in tqdm(df.groupby(['worker_id', 'strategy', 'session', 'condition', 'l_length', 'pres_rate'])):
...
def intrusion_rates(df, condition_map, toggle=True):
    intr_data = []
    for (sub, strat, sess, c, ll, pr), data in tqdm(df.groupby(['worker_id', 'strategy', 'session', 'condition', 'l_length', 'pres_rate'])):
...
def irt_final(df, condition_map, mwr_md):
    ...
    irt_data = irt_data[['subject', 'strategy', 'session', 'condition', 'l_length', 'pres_rate'] + [f'rot_{x}' for x in np.arange(max(res.lb)-1, -1, -1)]]
...
def irt_final_btwn_subj_avg(irt_final_data):
    ...
    # store results in dataframe
    irt_final_data_bsa = pd.DataFrame(irt_final_data_bsa, columns=['subject', 'strategy', 'condition', 'l_length', 'pres_rate'] + [f'rot_{x}' for x in np.arange(max(res.lb)-1, -1, -1)])
...
def irt_tot(df, condition_map):
    irt_tot_data = []
    for (sub, strat, sess, c, ll, pr), data in tqdm(df_strat.groupby(['worker_id', 'strategy', 'session', 'condition', 'l_length', 'pres_rate'])):
